...
class WeightLoader(object):
    ...
    def load_upsample_scales(self, upsample_params):
        ...
        scale_input = helper.make_tensor_value_info(name, TensorProto.FLOAT,
                                                    shape)
    ...
    def _create_param_tensors(self, conv_params, param_category, suffix):
        ...
        initializer_tensor = helper.make_tensor(param_name, TensorProto.FLOAT,
                                                param_data_shape, param_data)
        input_tensor = helper.make_tensor_value_info(param_name,
                                                     TensorProto.FLOAT,
                                                     param_data_shape)
    ...
    def _load_one_param_type(self, conv_params, param_category, suffix):
        ...
        param_data = np.ndarray(shape=param_shape,
                                dtype='float32',
...
class GraphBuilderONNX(object):
    ...
    def _make_conv_node(self, layer_name, layer_dict):
        ...
        if batch_normalize:
            ...
            batchnorm_node = helper.make_node('BatchNormalization',
                                              inputs=inputs,
        ...
        if layer_dict['activation'] == 'leaky':
            ...
            lrelu_node = helper.make_node('LeakyRelu',
                                          inputs=inputs,
    ...
    def _make_upsample_node(self, layer_name, layer_dict):
        ...
        scales = np.array([1.0, 1.0, upsample_factor,
                           upsample_factor]).astype(np.float32)
        previous_node_specs = self._get_previous_node_specs()
        inputs = [previous_node_specs.name]
        ...
        if checksum != checksum_reference:
            raise ValueError(
                'The MD5 checksum of local file %s differs from %s, please manually remove \
                 the file and try again.' % (local_path, checksum_reference))
    return local_path
